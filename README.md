...
It applies, one at a time, edits written by hand in the repository's own `tests/defects.sh` and requires the suite to notice. Two verbs write them: `defect` replaces text that appears exactly once, and `plant` covers what a replacement cannot express — a line appended, a file created, rewritten whole or removed — each with its own way of reporting that it has stopped being an edit at all. Either verb takes `--and FILE FIND REPLACE` for one defect that needs several edits at once, because a guard whose halves are both needed says nothing when only one of them is broken. **Nothing is generated** — tools that invent mutants produce hundreds per change and made themselves useful by suppressing them down to a handful; a hand-written list starts there, with the one thing only a person has, what each guard is for
...
The lint half runs the [skill-authoring](https://github.com/rokokol/skill-authoring-skill) skill's gate for a skill repository, `check-skill.sh`, and the [ci](https://github.com/rokokol/ci-skill) skill's pin guard, `check-pins.sh`, both vendored, lints every script, holds every document to one paragraph per line, and reads the marker files exactly as `run` reads them: every entry must catch a line in its set's lying fixture and stay quiet on a real healthy run of its tool, kept under `tests/fixtures/clean/`, so a dead marker cannot sit there looking like a guard and a noisy one cannot redden good runs
...
Then every check is proven able to fail. Most of that lives in this repository's own `tests/defects.sh` and runs after the merge: 65 entries, grouped `behaviour/` and `lint/` by the half that has to catch them, each naming the message that must do the catching. Six stay in the gate, where a copy of the repository is made per planted defect and each copy must fail for its own defect's reason: one edits `check.sh`, which is the suite; three edit fixtures under `tests/`, which `falsify` refuses and would have to be excused for every entry at once; and two are planted only under the bash 3.2 a macOS runner has. Every one of them was watched failing first — a dozen found real bugs in this repository while being written, three of them only under a real bash 3.2, and the list found one more on its first run, in the harness it had just moved out of
