...
- **A test pins the behaviour, not a copy of the code or its output.** An expected value copied whole from the output, a tuning constant restated, an exact message or layout the code is free to change: each is a second source of truth that fails on a harmless edit and teaches everyone to update the test without reading it. Assert the facts the behaviour promises; compare exactly only where the exact form is itself the contract, such as an exit code, a wire format another program parses or a file format on disk. This is not a weaker check: it is proven the same way, by a broken behaviour turning it red and a reworded output leaving it green. See [references/oracle.md](references/oracle.md)
