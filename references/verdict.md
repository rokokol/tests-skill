...
**A check whose clean result is silence needs a third condition: it has to say what it examined.** Where "nothing wrong" and "nothing examined" both print nothing and exit 0, the two outputs are identical and no amount of log reading separates them — an input list that came out empty, a glob that matched nothing, a path that was not there. Measured: a sweep over twenty-two repositories reported no findings at exit 0 because the shell running it did not word-split the list of paths, so `find` was handed one path that did not exist and the sweep read zero files. Two ways out, and both are in use: count what was examined and print the count, as `check-sh.sh` does when it ends a run with the number of planted defects it caught, or refuse outright, as `check-pins.sh` does by exiting 2 when it finds no workflows — a guard that scanned nothing is not a pass. A check that can only be silent is run against a known-positive in the same invocation, so silence stops being ambiguous
