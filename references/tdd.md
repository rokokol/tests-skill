...
1. **Red.** Write one test for one behaviour. Run it. It must fail, and the failure message must be the one you expected — `AssertionError: expected 3, got 2`, not `ImportError` or `fixture 'client' not found`. A test failing for a boring reason has not tested anything yet.
2. **Green.** Write the smallest code that makes it pass. Resist writing the next three things you know are coming; they get their own cycles and their own red.
...
4. **Repeat**, one behaviour at a time, keeping a todo item per cycle so the loop survives an interruption.
...
- **One behavioural claim per test**, named as the claim: `silence_before_the_first_word_is_trimmed`, not `test_trim_2`.
