...
- **A negative assertion on a message string dies the day the message changes, and dies silently.** `expect(out).not.toMatch('1 obsolete snapshot found')` passes when the guard works, and passes just as well when the wording moved, when the reporter started writing to stderr, or when the run produced no output at all. Jest's own end-to-end suite carries three of these against a string its 23.0.0 release removed in 2018; they have asserted nothing since, and nothing said so — a positive assertion beside them failed loudly the moment the same wording was touched. Assert what the output *does* say, and when the absence of something is genuinely the claim, pin it to a shape the tool commits to rather than to prose: jest's `Snapshots:   1 obsolete, 0 total` totals line outlived two rewordings of the sentence above it
...
- **Name what should do the catching, where it matters.** `expect caught FRAGMENT` holds a `caught` to more than the suite having gone red: the run's output has to carry FRAGMENT, a test name or the message that assertion prints. "The suite went red" and "the suite noticed this" are different claims, and one flaky test failing through a whole run credits every defect to a suite that saw none of them. Caught while FRAGMENT is absent is `stale`, the same as a renamed test, because the entry has stopped describing what it is about
...
A survivor is closed by a test written from what the guard promises — the help, the documentation, the contract its caller relies on — and never from what the code does now, which is the one source that cannot disagree with itself. Most such tests go green on the unbroken code: the behaviour was right and nothing checked it. One that goes red on the unbroken code has found a bug in the code, not a gap in the suite — a help that promises exit 2 on a usage error, a survivor on the refusal, and a test written from the help that meets an exit 1 — and that bug is fixed test first, in its own commit, before the entry counts as caught. This is where a defect list pays most: the survivor says where to look, and the specification says what should be there
