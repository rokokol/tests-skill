...
- **The failing test is its own commit, and the fix is the next one.** The history then contains a commit where the bug is demonstrated and one where it is gone, which is the clearest possible record of what was wrong. Bisect lands on the fix, and its diff is exactly the change that mattered.
...
What a bisect needs from the person running it: a reliable reproducer. If the check is itself unstable, bisect will confidently return a random commit — see [flaky.md](flaky.md), and run `t.sh flaky` on the reproducer first if there is any doubt.
