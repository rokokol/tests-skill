...
- **jest names the code it is about to exit with, and on 30.5.0 it names the right one.** `--passWithNoTests` gives `No tests found, exiting with code 0` and an exit of 0; without it, code 1 and an exit of 1. This entry used to say the line contradicted the status, and it no longer does — the useful part is that the sentence tells you which of the two happened, so a log carrying `exiting with code 0` is a run that found nothing and was told not to mind
